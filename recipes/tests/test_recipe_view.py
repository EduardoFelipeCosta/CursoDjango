...
class RecipesViewsTests(RecipeTestBase):
    ...
    def test_recipe_category_template_loads_recipes(self):
        needed_title = 'This is a recipe category'
        self.make_recipe(title=needed_title)
        response = self.client.get(reverse('recipes:category', args={1,}))
        content = response.content.decode('utf-8')
        respomse_context_recipes = response.context['recipes']
        self.assertIn(needed_title, content)  
        self.assertEqual(len(respomse_context_recipes), 1)
    ...
    def test_recipe_home_category_view_returns_404_if_no_recipes_found(self):
        category = Category.objects.create(name='Categoria vazia')
